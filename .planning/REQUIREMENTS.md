...
Summerfield DDEV clone at `/home/upan/projects/sf_staging/`. Apache + PHP 8.4 + MySQL 8.0 + WP 6.9+ + WC 8.x+.

- [ ] **DEV-01**: DDEV project running; `https://summerfield.ddev.site` accessible; mkcert certs trusted by browser
...
- [ ] **DEV-03**: `kmn-revenue-abilities` plugin symlinked from `PORTAL/wordpress-plugins/kmn-revenue-abilities/` into `wp-content/plugins/`; activated successfully
- [ ] **DEV-04**: Maxi AI plugin (`PORTAL/maxi-ai/`) symlinked and activated; coexists with kmn plugin without collision
...
- [ ] **DEV-09**: `.gitignore` updated: `wp-local/` style path not relevant (wp lives in WSL home), but `wordpress-plugins/*/.phpunit.result.cache` + `wordpress-plugins/*/composer.lock` added
