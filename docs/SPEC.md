...
Design tokens are the primary source of truth — see `src/shared/styles/tokens.css`.
...
│   ├── TaskCard.tsx
│   ├── TaskDetail.tsx
│   ├── TaskDetailSheet.tsx    # Sheet (URL state ?taskId=xxx)
│   ├── TaskComments.tsx
│   ├── TaskActions.tsx        # Approve, Request Changes, Hold, Cancel
│   ├── TaskFilters.tsx
│   ├── TaskFilterPanel.tsx
│   ├── TaskSearchBar.tsx
│   ├── NewTicketDialog.tsx    # mode="ticket" | mode="project"
│   ├── PriorityIcon.tsx       # Volume-bar style (1/2/3 bars + AlertCircle)
│   ├── SupportChat.tsx
│   ├── SupportSheet.tsx
│   ├── CommentInput.tsx
...
│   └── SyncIndicator.tsx
├── hooks/
│   ├── useClickUpTasks.ts     # task_cache via React Query + Realtime
...
│   ├── useCreateTask.ts
│   ├── useSingleTask.ts
│   └── useSupportTaskChat.ts
├── lib/
│   ├── status-mapping.ts      # ClickUp → Portal mapping
│   ├── status-dictionary.ts   # STATUS_LABELS, PRIORITY_LABELS
│   └── transforms.ts          # transformCachedTask
├── types/tasks.ts
└── pages/TicketsPage.tsx (Sheet-based), SupportPage.tsx
```

Shared components used by tickets (live in `src/shared/components/`):
- `common/MessageBubble.tsx` — chat bubble component
- `common/StatusBadge.tsx` — portal status badge
- `common/EmptyState.tsx` — empty state with icon + message
- `inbox/NotificationAccordionItem.tsx` — mobile inbox accordion
- `inbox/NotificationDetailPanel.tsx` — desktop notification detail
