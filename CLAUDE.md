...
| `supabase/functions/_shared/notifications.ts` | Bell gating helpers: `shouldCreateBell(profile, bellEvent)` — gates in-app bell creation by `notification_preferences` JSONB, mirrors `shouldSendEmail`. `BELL_EVENT_TO_PREF_KEY` — 14 bell events → 12 preference keys. Fail-open for unmapped events. Used by clickup-webhook (14 sites) and post-task-comment (1 site). |
...
| `supabase/functions/update-task-status/index.ts` | Client action handler: approve, request_changes, put_on_hold, resume, cancel, approve_credits, accept/decline_recommendation. `approve_credits` uses `upsert_task_deduction` RPC for atomic credit UPSERT and writes `task_cache.approved_credits` after each approval. |
| `supabase/migrations/20260417100000_credit_reapproval.sql` | Adds `task_cache.approved_credits numeric`; backfills from credit_transactions; creates `upsert_task_deduction` RPC (SECURITY DEFINER, service_role-only, partial-index UPSERT). |
