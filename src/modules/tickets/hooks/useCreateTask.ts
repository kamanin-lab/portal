  ...
  return useMutation({
    mutationFn: async (input: CreateTaskInput): Promise<CreateTaskResponse> => {
      ...
      const body: Record<string, unknown> = {
        name: input.name, description: input.description, priority: input.priority, files,
      };
      if (input.listId) body.listId = input.listId;
      if (input.phaseFieldId) body.phaseFieldId = input.phaseFieldId;
      if (input.phaseOptionId) body.phaseOptionId = input.phaseOptionId;

      const { data, error } = await supabase.functions.invoke('create-clickup-task', { body });
